# 딥러닝 메인 로직 외에 유틸리티성 기능들을 모아 놓은 로직
...
## 인자로 받은 이미지의 bbox를 그리는 함수 (정보값을 그리는 함수)
...
## opencv의 rectangle function을 이용해서 이미지 내의 절대좌표로 표현된 xmin,xmax,ymin,ymax 위치의 bbox를 그려줌
...
## xmin,ymin 위치 쪽에 label 정보와 소수점 셋째자리까지 잘린 confidence값을 한칸 띄고 표현
...
## bbox의 정보 (ex: 7x7x2 =98)를 받아와서 bbox정보를 기준으로 confidence기준으로 내림차순으로 전체 bbox 98개를 sorting을 한다음 거기에서 'confidence가 가장 큰 bbox 하나'만 리턴하는 NMS의 약식버젼
...
## yolo format을 draw_bounding_box에 쓰이는 cv 형태 format으로 변경
def yolo_format_to_bounding_box_dict(xcenter, ycenter, box_w, box_h, class_name, confidence):
  bounding_box_info = {}
  bounding_box_info['left'] = int(xcenter - (box_w / 2)) # xmin
  bounding_box_info['top'] = int(ycenter - (box_h / 2)) # ymin
  bounding_box_info['right'] = int(xcenter + (box_w / 2)) #xmax
  bounding_box_info['bottom'] = int(ycenter + (box_h / 2)) #ymax
...
## yolo가 predict한 box정보와 ground truth(정답) 정보를 받아서 2개에 대한 0~1로 표현되는 IOU를 계산해서 반환하는 함수 
def iou(yolo_pred_boxes, ground_truth_boxes):
  ...
  square1 = (boxes1[:, :, :, 2] - boxes1[:, :, :, 0]) * (boxes1[:, :, :, 3] - boxes1[:, :, :, 1]) # bbox 1에 대한 넓이 
  square2 = (boxes2[2] - boxes2[0]) * (boxes2[3] - boxes2[1]) ## bbox 2에 대한 넓이 

  return inter_square / (square1 + square2 - inter_square + 1e-6) ## 교집합/합집합



## 화면에 drawing할 때 class별로 컬러 값을 다르게 줘서 그리기 --> 클래스 별로 다른 컬러 값을 랜덤 값으로 할당함
